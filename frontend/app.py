...
from memory.retrieve import get_customer_subgraph
...
@app.route("/chat")
def chat():
    # Memory Q&A — ask the agent about customers, tactics, call history.
    return render_template("chat.html")


def _customer_system_prompt(ctx: dict) -> str:
    """Structured LLM system prompt from a get_customer_subgraph() result."""
    c = ctx["customer"]
    lines = [
        "You are Lowball Sales Intelligence — a B2B negotiation memory assistant for manufacturers.",
    ...
    for call in ctx.get("all_calls") or []:
        date = (call["ts"] or "")[:10]
        lines.append(f"  {date} | sentiment {call['sentiment']:+.2f} | outcome {call['outcome_score']:.2f}")
...
def api_chat():
    ...
    def mock():
        if ctx:
            ...
            return {"answer": f"[MOCK] Memory for {name} is loaded. Add OPENAI_API_KEY to .env for live answers."}
        return {"answer": "[MOCK] Negotiation data for all customers is loaded. Add OPENAI_API_KEY to .env for live answers."}
...
@app.route("/call")
def call():
    # In-browser call desk — the customer (you) dials the manufacturer's
    # ElevenLabs agent. The public ConvAI widget connects the browser mic
    # directly to the agent; no API key needed on this page.
